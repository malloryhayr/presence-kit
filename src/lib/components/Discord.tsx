...
const DEFAULT_LARGE_ASSET_OVERRIDES = {
	VALORANT:
		'https://cdn.discordapp.com/app-icons/700136079562375258/e55fc8259df1548328f977d302779ab7',
};
...
export default function Discord({
		...
		const interval = setInterval(() => {
			if (
				...
				const endTime = Math.floor(
					(activity.spotify.timestamps.end - activity.spotify.timestamps.start) / 1000
				);
				const endFormatted = `${Math.floor(endTime / 60)}:${Math.floor(endTime % 60).toLocaleString(
					'en-US',
				...
				setSpotifyProgress(
					((Date.now() - activity.spotify.timestamps.start) /
						(activity.spotify.timestamps.end - activity.spotify.timestamps.start)) *
						100
				);
			}
	...
	function getFormattedTimestamp(start: number) {
		...
		return `${
			...
			Math.floor(current / 60) - Math.floor(Math.floor(current / 60) / 60) * 60
		).toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })}:${Math.floor(
			current % 60
			...
			if (activity.active_on_discord_desktop) {
				return (
					<>
						<StatusCircle
							status={
								activity.discord_status === 'dnd' || activity.discord_status === 'idle'
									? activity.discord_status
									: activity.active_on_discord_desktop
									? 'online'
									: 'offline'
							}
							bg={bgStyle}
					...
					<>
						<MobileStatus
							status={
								activity.discord_status === 'dnd' || activity.discord_status === 'idle'
									? activity.discord_status
									: activity.active_on_discord_mobile
									? 'online'
									: 'offline'
							}
							bg={bgStyle}
			...
			return (
				<>
					<Row style={{ paddingBottom: '8px' }}>
						<ImageContainer>
							<AvatarImage
								src={`https://cdn.discordapp.com/avatars/${activity.discord_user.id}/${
									activity.discord_user.avatar
								}${activity.discord_user.avatar.startsWith('a_') ? '.gif' : '.png'}`}
									...
									<Info>
										<p>
											{getFormattedTimestamp(
												activityData?.timestamps?.start ? activityData.timestamps.start : 0
											)}
										</p>
									</Info>
								</InfoContainer>
					...
					if (activity.spotify) {
						return (
							<div key={index}>
								...
								<Row
									style={index < activity.activities.length - 1 ? { paddingBottom: '16px' } : {}}
								>
									<ProgressContainer>
				...
				case 4: {
					if (activityData.emoji && activityData.emoji.id) {
						return (
							<div key={index}>
								<Row>
									...
									>
										<DiscordEmoji
											id={activityData.emoji.id}
											animated={activityData.emoji.animated}
										/>
										{activityData.state}
									</ActivityHeader>
								</Row>
						...
						return (
							<div key={index}>
								<Row>
									<ActivityHeader style={{ fontWeight: 400 }}>
										<Twemoji
											style={{ display: 'flex', alignItems: 'center', paddingBottom: '10px' }}
											text={textStyle}
										>
											{activityData.emoji ? activityData.emoji.name : ''}
											{activityData.state}
										</Twemoji>
									</ActivityHeader>
								</Row>
	...
	if (activity) {
		...
		return (
			<>
				<DiscordContainer bg={bgStyle} text={textStyle} border={border} style={style}>
					{textStyle === DiscordTextStyle.LIGHT ? (
						<DiscordLoader backgroundColor="#2b2b2b" foregroundColor="#424242" />
					) : (
						<DiscordLoader backgroundColor="#c9c9c9" foregroundColor="#ffffff" />
					)}
				</DiscordContainer>
			</>
		);
...
const ProgressBackground = styled.div<StatusStyleProps>`
	background-color: ${props =>
		props.text === DiscordTextStyle.LIGHT ? 'rgba(200, 200, 200, 0.3)' : 'rgba(79, 84, 92, .16)'};
	height: 5px;
	border-radius: 5px;
`;
